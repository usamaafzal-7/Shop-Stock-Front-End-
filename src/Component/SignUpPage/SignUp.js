...
const SignUp = () => {
  ...
  const {
    ...
    onSubmit: async (value, action) => {
      ...
      try {
        const res = await axios.post(`${process.env.REACT_APP_HOST}api/User`, value);
        // console.warn(res, "dataaaa storeee resss");
        action.resetForm();
        navigate("/customer");
      } catch (error) {
        console.warn("Sign Upp account Error", error);
      }
    },
  });
        ...
        >
          ...
          <Typography>
            ...
            <Link to={"/customer"} style={{ textDecoration: "none" }}>
              <Button sx={{ ":hover": { color: "#eab709" } }}>Login</Button>
            </Link>
          </Typography>
        </Box>
      </Box>
